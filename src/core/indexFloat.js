...
export class IndexFloat {
    ...
    hitAt(x, y) {
        ...
        for (const plane of this.gpu.planes) {
            if (plane.kind !== "main" || plane.opacity < 0.01) continue;
    ...
    _onUp(e) {
        ...
        const dragged = dx * dx + dy * dy > CLICK_THRESHOLD * CLICK_THRESHOLD;
        this.isDragging = false;
        if (dragged || !this.onSelect) return;
        const image = this.hitAt(e.clientX, e.clientY);
        if (image !== null) this.onSelect(image);
    }
}
